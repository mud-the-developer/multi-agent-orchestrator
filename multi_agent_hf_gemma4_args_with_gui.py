...
class TraceStore:
    ...
    def _fire_webhook(self, event_type: str, record: Any) -> None:
        if not self.webhook_url:
            return
        payload = {"event": event_type, "data": asdict(record)}
        data_bytes = safe_json_dumps(payload).encode("utf-8")
        
        def _post():
            try:
                req = urllib.request.Request(self.webhook_url, data=data_bytes, headers={"Content-Type": "application/json"}, method="POST")
    ...
    def log_agent_step(self, record: AgentStepRecord) -> None:
        ...
        self._fire_webhook("agent_step", record)
...
class PipelineConfig:
    ...
    webhook_url: Optional[str] = "http://127.0.0.1:3123/api/hook"
...
def load_pipeline_config(
    ...
    return PipelineConfig(
        ...
        notes=notes,
        webhook_url=raw.get("webhook_url", base.webhook_url),
    )
...
def apply_cli_overrides(cfg: PipelineConfig, args: argparse.Namespace) -> PipelineConfig:
    ...
    return PipelineConfig(
        ...
        webhook_url=args.webhook_url or cfg.webhook_url,
    )
...
def main() -> None:
    ...
    parser.add_argument("--webhook-url", type=str, help="Webhook URL to POST trace events in real-time")
